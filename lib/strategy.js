...
/**
 * Creates an instance of `SchoologyStrategy`.
 *
 ...
 *   - `sgyDomain`             Domain at which Schoology service is hosted, usually https://<district>.schoology.com
 *
 * Examples:
 *
 *     passport.use(new SchoologyStrategy({
 ...
 * @api public
 */
function SchoologyStrategy(options, verify) {
    ...
    if (!verify) {
        throw new TypeError('SchoologyStrategy requires a verify callback');
    }
    if (!options.apiBase) {
        options.apiBase = 'https://api.schoology.com/v1'
    }
    if (!options.sgyDomain) {
        throw new TypeError('SchoologyStrategy requires a sgyDomain option');
    }
    if (!options.requestTokenURL) {
        options.requestTokenURL = `${options.apiBase}/oauth/request_token`
    }
    if (!options.accessTokenURL) {
        options.accessTokenURL = `${options.apiBase}/oauth/access_token`
    }
    if (!options.userAuthorizationURL) {
        options.userAuthorizationURL = `${options.sgyDomain}/oauth/authorize`
    }
    if (!options.consumerKey) {
        throw new TypeError('SchoologyStrategy requires a consumerKey option');
    }
    if (options.consumerSecret === undefined) {
        throw new TypeError('SchoologyStrategy requires a consumerSecret option');
    }
    ...
    this._oauth.setClientOptions(options.clientOptions || {
...
util.inherits(SchoologyStrategy, passport.Strategy);
 ...
 * @api protected
 */
SchoologyStrategy.prototype.authenticate = function (req, options) {
    options = options || {};
 ...
 * @api protected
 */
SchoologyStrategy.prototype.userProfile = function (token, tokenSecret, params, done) {
    return done(null, {});
};
 ...
 * @api protected
 */
SchoologyStrategy.prototype.requestTokenParams = function (options) {
    return {};
};
 ...
 * @api protected
 */
SchoologyStrategy.prototype.userAuthorizationParams = function (options) {
    return {};
};
 ...
 * @api protected
 */
SchoologyStrategy.prototype.parseErrorResponse = function (body, status) {
    return null;
};
 ...
 * @api private
 */
SchoologyStrategy.prototype._loadUserProfile = function (token, tokenSecret, params, done) {
    var self = this;
 ...
 * @api private
 */
SchoologyStrategy.prototype._createOAuthError = function (message, err) {
    var e;
...
// Expose constructor.
module.exports = SchoologyStrategy;
